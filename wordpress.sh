...
PMA_HOST=$(echo $SECRET_JSON | grep -oP '(?<="PMA_HOST":")[^"]*')
PMA_PORT=$(echo $SECRET_JSON | grep -oP '(?<="PMA_PORT":")[^"]*')
PMA_USER=$(echo $SECRET_JSON | grep -oP '(?<="PMA_USER":")[^"]*')
PMA_PASSWORD=$(echo $SECRET_JSON | grep -oP '(?<="PMA_PASSWORD":")[^"]*')
...
sudo sed -i "s/localhost/$PMA_HOST/" /usr/share/nginx/html/phpmyadmin/config.inc.php

sudo systemctl restart nginx
sudo systemctl restart php-fpm
sudo systemctl restart mariadb

echo "WordPress and phpMyAdmin installation completed."